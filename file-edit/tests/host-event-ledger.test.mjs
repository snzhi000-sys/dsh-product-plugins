...
function harness(pluginImpl = plugin, sessionOverrides = {}, policyMode = 'workspace-write', pluginConfig = {}, services = {}) {
  ...
  const ctx = {
    ...
    sessions: {
      get: id => sessionRecords.get(id),
      list: async () => ({ items: [...sessionRecords.entries()].map(([id, record]) => ({ id, cwd: record.header.cwd })) }),
    },
...
test('a change inside another Session nested workspace never becomes this Session review', async () => {
  const nested = join(workspace, 'other-workspace')
  mkdirSync(nested, { recursive: true })
  const h = harness(plugin, { 'session-other': { header: { id: 'session-other', cwd: nested } } })
  writeFileSync(join(workspace, 'mine.txt'), 'before\n')
  await h.invoke('getModified', { sessionId: 'session-test' })
  const exec = {
    name: 'bash', callId: 'call-foreign-1', arguments: { command: 'du -sk .' },
    agent: { session: { id: 'session-test' } }, signal: new AbortController().signal,
  }
  await h.events.get('tools/execute')(exec, async () => {
    // The other Session's build writes inside its own (nested) workspace while this command runs.
    writeFileSync(join(nested, 'build-output.js'), 'built\n')
    // Positive control: a change in this Session's own workspace during the same window.
    writeFileSync(join(workspace, 'mine.txt'), 'after\n')
    return { isError: false, content: [], value: { kind: 'foreground', exitCode: 0 } }
  })
  const mine = (await h.invoke('getModified', { sessionId: 'session-test' })).files.map((f) => f.path)
  assert.ok(mine.includes('mine.txt'), JSON.stringify(mine))
  assert.equal(mine.some((path) => path.includes('build-output.js')), false, JSON.stringify(mine))
  // The other Session never attributed it either: it is baseline there, not its review.
  const other = (await h.invoke('getModified', { sessionId: 'session-other' })).files.map((f) => f.path)
  assert.deepEqual(other, [])
})

test('a foreign entry recorded before the ownership rule is pruned on the next read', async () => {
  const nested = join(workspace, 'other-workspace')
  mkdirSync(nested, { recursive: true })
  writeFileSync(join(nested, 'stale-output.js'), 'built\n')
  const ledgerDir = join(stateHome, 'dsh-file-edit-state')
  mkdirSync(ledgerDir, { recursive: true })
  const entry = (present, version, note) => ({ present, content: present ? 'built\n' : null, eol: present, crlf: false, version, size: 6, note, binRef: null, binSize: 0, md: false })
  writeFileSync(join(ledgerDir, 'session-test.json'), JSON.stringify({
    version: 7, root: workspace, baseReady: false, treeStamp: 0, mutationStamp: 1, auditCoverage: null, lastReject: null,
    files: {
      'other-workspace/stale-output.js': {
        base: entry(true, 'shell:call-old:before-unknown', 'shell-unknown'),
        cur: entry(true, '1:6', undefined),
        rev: 1, decisions: {}, deletion: null, deletedPreview: null,
      },
    },
  }))
  const h = harness(plugin, { 'session-other': { header: { id: 'session-other', cwd: nested } } })
  const listed = (await h.invoke('getModified', { sessionId: 'session-test' })).files.map((f) => f.path)
  assert.deepEqual(listed, [])
  const saved = JSON.parse(readFileSync(join(ledgerDir, 'session-test.json'), 'utf8'))
...
test('a mutating str_replace_editor call enters the ledger from its own pre-image', async () => {
  const h = harness(plugin, { 'session-editor': { header: { id: 'session-editor', cwd: workspace } } })
  const path = 'editor-target.md'
  writeFileSync(join(workspace, path), 'one\n')
  const exec = {
    name: 'str_replace_editor',
    callId: 'call-editor-1',
    arguments: { command: 'str_replace', path: join(workspace, path), old_str: 'one', new_str: 'two' },
    agent: { session: { id: 'session-editor' } },
  }
  await h.events.get('tools/pre-execute')(exec, async () => ({ isError: false }))
  writeFileSync(join(workspace, path), 'two\n')
  await h.events.get('tools/result')(exec, { isError: false, value: 'The file has been edited successfully.' })
  const entry = stateForSession('session-editor').files[path]
  assert.equal(entry.base.content, 'one\n')
  assert.equal(entry.cur.content, 'two\n')
  const listed = (await h.invoke('getModified', { sessionId: 'session-editor' })).files.map((f) => f.path)
  assert.ok(listed.includes(path), JSON.stringify(listed))
})


test('str_replace_editor create starts from an absent baseline', async () => {
  const h = harness(plugin, { 'session-editor-create': { header: { id: 'session-editor-create', cwd: workspace } } })
  const path = 'editor-created.md'
  const exec = {
    name: 'str_replace_editor',
    callId: 'call-editor-2',
    arguments: { command: 'create', path: join(workspace, path), file_text: 'born\n' },
    agent: { session: { id: 'session-editor-create' } },
  }
  await h.events.get('tools/pre-execute')(exec, async () => ({ isError: false }))
  writeFileSync(join(workspace, path), 'born\n')
  await h.events.get('tools/result')(exec, { isError: false, value: 'New file created successfully' })
  const entry = stateForSession('session-editor-create').files[path]
  assert.equal(entry.base.present, false)
  assert.equal(entry.cur.content, 'born\n')
})


test('a str_replace_editor read or a failed call never enters the ledger', async () => {
  const h = harness(plugin, { 'session-editor-idle': { header: { id: 'session-editor-idle', cwd: workspace } } })
  const path = 'editor-untouched.md'
  writeFileSync(join(workspace, path), 'keep\n')
  const view = {
    name: 'str_replace_editor',
    callId: 'call-editor-view',
    arguments: { command: 'view', path: join(workspace, path) },
    agent: { session: { id: 'session-editor-idle' } },
  }
  await h.events.get('tools/pre-execute')(view, async () => ({ isError: false }))
  await h.events.get('tools/result')(view, { isError: false, value: '     1\tkeep' })
  const failed = {
    name: 'str_replace_editor',
    callId: 'call-editor-failed',
    arguments: { command: 'str_replace', path: join(workspace, path) },
    agent: { session: { id: 'session-editor-idle' } },
  }
  await h.events.get('tools/pre-execute')(failed, async () => ({ isError: false }))
  await h.events.get('tools/result')(failed, { isError: true, value: 'No match found' })
  const listed = (await h.invoke('getModified', { sessionId: 'session-editor-idle' })).files
  assert.deepEqual(listed, [])
})


test('an uncaptured shell entry point is disclosed instead of denied or dropped', async () => {
  const h = harness(plugin, { 'session-gaps': { header: { id: 'session-gaps', cwd: workspace } } })
  const background = {
    name: 'bash',
    callId: 'call-gap-background',
    arguments: { command: 'sleep 30; echo later > late.txt', run_in_background: true },
    agent: { session: { id: 'session-gaps' } },
  }
  const untracked = {
    name: 'shell_command',
    callId: 'call-gap-untracked',
    arguments: { command: 'echo x > compat.txt' },
    agent: { session: { id: 'session-gaps' } },
  }
  let backgroundRan = false
  await h.events.get('tools/execute')(background, async () => { backgroundRan = true; return { isError: false } })
  let untrackedRan = false
  await h.events.get('tools/execute')(untracked, async () => { untrackedRan = true; return { isError: false } })
  // Both calls still run: the gap is disclosed, not denied.
  assert.equal(backgroundRan, true)
  assert.equal(untrackedRan, true)
  const persisted = stateForSession('session-gaps')
  assert.equal(persisted.auditGaps.backgroundShell, 1)
  assert.deepEqual(persisted.auditGaps.untrackedShell, ['shell_command'])
  const modified = await h.invoke('getModified', { sessionId: 'session-gaps' })
  assert.deepEqual(modified.files, [])
  assert.deepEqual(modified.auditGaps.untrackedShell, ['shell_command'])
  // Repeats count again, and the disclosure survives a restart.
  await h.events.get('tools/execute')(background, async () => ({ isError: false }))
  assert.equal(stateForSession('session-gaps').auditGaps.backgroundShell, 2)
  const reboot = harness(plugin, { 'session-gaps': { header: { id: 'session-gaps', cwd: workspace } } })
