...
import {
  decodeFileReference, encodeFileReference, fileReferenceClipboardText, fileReferenceModelText,
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // 文件/文件夹图标一律用官方那套（与 Explorer 同源）：模块表里的基线平台模块，
    // 键名就是包名。分类、配色、代码语言图标都由官方组件自己负责。
    const { FileTypeIcon, IconFolderClose16, IconFolderOpen16 } = require('@deepseek-ai/dsh-client-ui-primitives')
      ...
      apply(ctx) {
        ...
        const activateFilesView = (sid) => {
          ...
          try {
            ...
            if (sidebar && typeof sidebar.openTab === 'function') {
              sidebar.openTab(FILE_BROWSER_KIND)
              return true
            }
            ...
            // 侧栏标签还没注册、或会话作用域取不到服务：报告未展示，让调用方回落。
            return false
          }
        }
        ...
        if (inputTriggers) {
          const fileRefCodec = {
            ...
            // 模型文本 = 官方 `@path` mention（+ 行范围文字）。正文不内联：
            // 选中行由范围指明，模型自己去读，因此不再需要 host readLines。
            serialize(ref) {
              return Promise.resolve(fileReferenceModelText(decodeFileReference(ref)))
            },
          }
        ...
        const fileReferenceInsert = (ref, kind, displayName) => {
          ...
          return {
            ...
            // 官方 @ 引用插入时同样声明的域图标：缺省会让 chip 渲染成 '@' 记号，
            // 声明后才是文件/文件夹图标与可打开态样式。
            appearance: decoded.kind === 'folder' ? 'folder' : 'file',
        ...
        const insertFileReference = (insert) => {
          ...
          // 光标必须来自官方输入面自己的 detect 坐标。composer 自 shell 迁移起就是
          // Lexical contenteditable，旧的 `textarea[data-phase]` 探测只会拿到 null 并
          // 退化成位置 0——那样每次引用都插到草稿最前面，无法在对话中途穿插引用。
          // 调用方传入的 textarea 偏移同样作废，忽略即可（多余实参在 JS 里无害）。
          let caret = { start: 0, end: 0 }
          try {
            const live = typeof input.caretSpan === 'function' ? input.caretSpan() : null
            if (live && Number.isInteger(live.start) && Number.isInteger(live.end) && live.start >= 0) caret = live
          } catch (e) {
            console.warn('[dsh-file-edit] 读光标失败:', e)
          }
          const span = { start: caret.start, end: Math.max(caret.start, caret.end), draftRev: draftRev }
          try {
            pendingInlineCaret = caret.start + 2
        ...
        // [文件引用] 工作区内的官方引用：与输入框自己的 @ 菜单同源——官方 `reference` source、
        // 官方 mention 语法（`@path`；含空白 `@"path`／`@"path"`；目录保留结尾斜杠）与公开的
        // `SessionInput.insertReference`。span 用 detect 坐标的草稿末尾：chip 在 detect 空间只占
        // 一个字符，`state.draft` 却是 chip 展开后的剪贴板投影，故用 occurrences 换算。
        const officialMention = (target, kind) => {
          const raw = String(target || '')
          if (!raw) return undefined
          if (/[\u0000-\u001f\u007f-\u009f"]/u.test(raw)) return undefined
          const path = kind === 'folder' ? raw.replace(/\/+$/, '') + '/' : raw
          if (!/\s/u.test(path)) return '@' + path
          return kind === 'folder' ? '@"' + path : '@"' + path + '"'
        }
        const insertOfficialReference = (entry) => {
          if (!entry || !entry.path) return false
          const mention = officialMention(entry.path, entry.kind)
          if (!mention) return false
          const sid = store.sessionId || ctx.sessions.list.getSnapshot().current
          if (!sid) return false
          try {
            const actx = ctx.sessions.scope(String(sid))
            const conversation = actx && actx.get('conversation')
            const input = conversation && conversation.input ? conversation.input.for(actx) : null
            if (!input || !input.state || typeof input.insertReference !== 'function') return false
            const snap = input.state.getSnapshot()
            const expanded = (snap.occurrences || []).reduce((extra, occurrence) => extra + Math.max(0, (occurrence && occurrence.length ? occurrence.length : 0) - 1), 0)
            const end = Math.max(0, String(snap.draft || '').length - expanded)
            return input.insertReference({
              source: 'reference',
              ref: mention,
              label: entry.label || referenceFileName(entry.path),
              appearance: entry.kind === 'folder' ? 'folder' : 'file',
              clipboardText: mention,
            }, { start: end, end: end, draftRev: snap.draftRev }) === true
          } catch (e) {
            console.warn('[dsh-file-edit] 官方引用插入失败:', e)
            return false
          }
        }
        ...
        Object.assign(store, {
          ...
          active: null,
          tabRevealTick: 0,
          // v1.14: 打开文件时可能带一个目标行（引用/审核列表给的定位）。视图渲染到那一行
          // 之后由 claimFileTarget 消费掉，所以后续轮询与编辑不会再把滚动位置抢回去。
          fileTargets: new Map(),
          // 目标行的版本号：同一个文件被再次带行打开时也要让视图重新执行一次定位。
          targetEpoch: 0,
          modified: new Set(),
          deleted: new Set(),
          draftDirty: new Set(),
          ...
          setSessionId(id) {
            ...
            if (this.sessionId) {
              sessionViews.set(this.sessionId, { tabs: this.tabs.slice(), active: this.active, deleted: new Set(this.deleted), modified: new Set(this.modified) })
            }
            ...
            // 修改标记也是逐会话的：否则切会话瞬间 tab 上的 ● 会沿用上一个会话的集合。
            this.modified = next && next.modified ? new Set(next.modified) : new Set()
            this.draftDirty = new Set()
            this.emit()
          },
          openFile(path, sessionId, displayPath, target) {
            if (!path) return
            if (sessionId) this.setSessionId(String(sessionId))
            if (displayPath) reviewLabels.set(path, displayPath)
            const line = target && Number(target.line) > 0 ? Math.floor(Number(target.line)) : 0
            const endLine = target && Number(target.endLine) >= line ? Math.floor(Number(target.endLine)) : line
            if (line > 0 || (target && target.firstChange)) {
              this.fileTargets.set(path, { line: line, endLine: endLine, firstChange: !!(target && target.firstChange) })
              this.targetEpoch++
            }
          ...
          // 目标行只交付一次：视图把它滚进视野后即取走，避免每次重渲染都重置滚动位置。
          // `firstChange` 用视图当时已知的首个变更行（审核列表点进来时用），没有就落到第 1 行。
          claimFileTarget(path, firstChangeLine) {
            const target = this.fileTargets.get(path)
            if (!target) return null
            this.fileTargets.delete(path)
        ...
        // [集成] 打开入口：同仓插件（Explorer）通过 Cordis 服务读取，服务带版本号，
        // 消费方按版本拒绝过老的提供方。页面全局对象只为更早的插件构建保留一个版本，
        // 且不再发布没有消费方的 rename/remove/markDeleted。
        const fileEditOpen = async (request) => {
          ...
          // Every refusal reports `false` so the caller can open the official
          // preview instead of leaving the click without a visible outcome.
          if (!result || !result.ok || !result.id) return false
          if (result.kind === 'binary') return false
          const current = ctx.sessions.list.getSnapshot().current
          if (current && String(current) !== sid) return false
          ...
          return activateFilesView(sid)
        }
        const fileEditOpenService = { version: 1, open: fileEditOpen }
        try {
          ctx.effect(() => ctx.provide('dshFileEditOpen', fileEditOpenService))
        } catch (e) {
          console.warn('[dsh-file-edit] 发布打开服务失败，Explorer 将回退到页面对象:', e)
        }
        if (typeof window !== 'undefined') {
          try {
            window.__dshFileEdit = { open: fileEditOpen }
          } catch (e) {}
        }
        ...
        let styleEl = null
        const EXTRA_CSS = [
          // 审核界面的"新增/接受"绿：配方沿用官方 state-success 的用法，色号
          // 换成更亮的一档（官方那档在审核文案上偏暗、不易读）。只作用于本插件
          // 的审核界面，官方 token 仍作回退，未取到变量时行为不变。
          ':root { --dsh-fe-review-add: #19BB4D; }',
          ...
          // —— 宽度基准对齐官方输入框（2026-09-16）——
          // 审核栏与官方输入框卡片是同一个 .composerStack 里的兄弟节点，所以直接照抄
          // InputBar 的实际几何：官方卡片（.pW7-CW_card）是 width:100%，但它外面的
          // .pW7-CW_root 带 padding:0 var(--dsh-composer-side-clearance)，于是卡片的
          // 真实宽度是 min(容器宽 - 2×side-clearance, card-max-width)。
          // 原来只写 width:100% 就漏掉了那圈 16px 边距：窄屏时审核栏正好比输入框宽
          // 32px（每侧 16px），并且先顶到容器边缘、跟着缩不动，永远对不齐。
          // container-type 供下面的窄屏内容适配查询使用（官方 composer 行同样用
          // container-type:inline-size）。
          '.dsh-fe-bar { width:calc(100% - var(--dsh-composer-side-clearance, 16px) - var(--dsh-composer-side-clearance, 16px)); max-width:var(--dsh-composer-card-max-width, 100%); margin:0 auto; box-sizing:border-box; container-type:inline-size; }',
          ...
          '.dsh-fe-iconbtn-ok { color:var(--dsh-fe-review-add, var(--dsw-alias-state-success-primary)); }',
          '.dsh-fe-iconbtn-ok:hover { background:color-mix(in srgb, var(--dsh-fe-review-add, var(--dsw-alias-state-success-primary)) 16%, transparent); color:var(--dsh-fe-review-add, var(--dsw-alias-state-success-primary)); }',
          ...
          '.dsh-fe-stat-add { color:var(--dsh-fe-review-add, var(--dsw-alias-state-success-primary)); }',
          ...
          // 目标行的一次性高亮（1.2s 后由 JS 摘掉）：inset 阴影叠在既有行底色之上，
          // 不会盖掉新增/删除行的绿红底色，也不影响文字。
          '.dsh-fe-target { box-shadow:inset 0 0 0 9999px color-mix(in srgb, var(--dsw-alias-state-warn-primary) 14%, transparent); }',
          // Diff tint sits on the WHOLE row (full width, gutter column keeps
          // its own surface via .dsh-fe-ln's solid background) — v1.8.1.
          '.dsh-fe-line.dsh-fe-old { background:color-mix(in srgb, var(--dsw-alias-state-error-primary) 12%, transparent); }',
          '.dsh-fe-line.dsh-fe-new { background:color-mix(in srgb, var(--dsh-fe-review-add, var(--dsw-alias-state-success-primary)) 12%, transparent); }',
          '.dsh-fe-line.dsh-fe-old .dsh-fe-tx, .dsh-fe-line.dsh-fe-new .dsh-fe-tx { background:none; }',
          // 删除/新增行的文字与符号改用语义色，配方照官方审核栏（12% 底色 +
          // state token 文字）。行内语法高亮 span 一并跟随，否则整行读不出
          // 删除/新增含义。
          '.dsh-fe-line.dsh-fe-old .dsh-fe-tx, .dsh-fe-line.dsh-fe-old .dsh-fe-tx .dsh-fe-tk { color:var(--dsw-alias-state-error-primary); }',
          '.dsh-fe-line.dsh-fe-new .dsh-fe-tx, .dsh-fe-line.dsh-fe-new .dsh-fe-tx .dsh-fe-tk { color:var(--dsh-fe-review-add, var(--dsw-alias-state-success-primary)); }',
          ...
          // ---- 卡片外观对齐官方 composer 对话框（2026-09-16）----
          // 官方卡片（ui-conversation/skeleton/InputBar.module.css 的 .card）：border:0，
          // 描边是 0.5px 的仰角环（--dsw-elevation-stroke-color: --dsw-alias-border-l2，
          // 由 --dsw-elevation-soft 画出，不占布局），border-radius:22px，软阴影。
          // 卡片的宽度与 max-width 早已与 composer 一致（--dsh-composer-card-max-width）。
          '.dsh-fe-bar { border:0; --dsw-elevation-stroke-color:var(--dsw-alias-border-l2); border-radius:22px; box-shadow:var(--dsw-elevation-soft); }',
          // ---- 审核栏里可点击项的悬停底色与形状，按官方规范（2026-09-16）----
          // 官方行（ui-workspace/rows/Rows.module.css）：border-radius:8px + background:
          // var(--dsw-alias-interactive-bg-hover)；官方圆钮（ui-dockkit 的 .addTab/.tabClose、
          // 侧栏 iconButton）是圆形、共用同一个交互底色，色调只落在墨色上。
          // 作用域限定在 .dsh-fe-bar，文件浏览器自己的行/按钮不受影响。
          '.dsh-fe-bar .dsh-fe-file-row, .dsh-fe-bar .dsh-fe-delete-batch-child { border-radius:12px; }',
          '.dsh-fe-bar .dsh-fe-file-row:hover, .dsh-fe-bar .dsh-fe-delete-batch-child:hover { background:var(--dsw-alias-interactive-bg-hover); }',
          // 状态标签用官方 Tag 的**实底**色阶（ui-primitives/Tag.module.css）：中性 bg-module-platform、
          // warning 12%、success/danger 10%，文字取同色系；一律无描边、全圆角胶囊。
          '.dsh-fe-bar .dsh-fe-chip { border:none; border-radius:999px; padding:1px 8px; font-size:11px; line-height:17px; font-weight:500; background:var(--dsw-alias-bg-module-platform); color:var(--dsw-alias-label-secondary); }',
          '.dsh-fe-bar .dsh-fe-chip-external { border:none; background:color-mix(in srgb, var(--dsw-alias-state-warn-primary) 12%, transparent); color:var(--dsw-alias-state-warn-primary); }',
          '.dsh-fe-bar .dsh-fe-chip-add { border:none; background:color-mix(in srgb, var(--dsh-fe-review-add, var(--dsw-alias-state-success-primary)) 10%, transparent); color:var(--dsh-fe-review-add, var(--dsw-alias-state-success-primary)); }',
          '.dsh-fe-bar .dsh-fe-chip-del { border:none; background:color-mix(in srgb, var(--dsw-alias-state-error-primary) 10%, transparent); color:var(--dsw-alias-state-error-primary); }',
          '.dsh-fe-bar .dsh-fe-iconbtn, .dsh-fe-bar .dsh-fe-secbtn { border-radius:999px; }',
          '.dsh-fe-bar .dsh-fe-iconbtn:hover, .dsh-fe-bar .dsh-fe-secbtn:hover { background:var(--dsw-alias-interactive-bg-hover); color:var(--dsw-alias-label-primary); }',
          '.dsh-fe-bar .dsh-fe-iconbtn-ok:hover { color:var(--dsh-fe-review-add, var(--dsw-alias-state-success-primary)); }',
          '.dsh-fe-bar .dsh-fe-iconbtn-no:hover { color:var(--dsw-alias-state-error-primary); }',
          // —— 审核栏墨色对齐官方 dock 卡片与官方文件树（2026-09-16）——
          // 审核栏是 composer 上方的 dock 卡片，官方同类表面把装饰性图元一律放在最弱的
          ...
          // 否则会把它们的色调一起盖成 tertiary。
          '.dsh-fe-bar .dsh-fe-iconbtn:not(.dsh-fe-iconbtn-ok):not(.dsh-fe-iconbtn-no) { color:var(--dsw-alias-label-tertiary); }',
          // hover 要多一档优先级才压得住上面那条常驻规则（同一元素上的 :not() 链也能匹配）。
          '.dsh-fe-bar .dsh-fe-iconbtn:not(.dsh-fe-iconbtn-ok):not(.dsh-fe-iconbtn-no):hover { color:var(--dsw-alias-label-secondary); }',
          // 收起时不要再留 body 上方那段 gap：否则上边距 8px、下边距 8px+4px，表头会偏上。
          '.dsh-fe-bar-collapsed { gap:0; }',
          ...
          // enter/leave.
          // 卡片最高高度：原来是 40vh（803px 高的窗口里实测 321px，14 行时占掉近半个屏幕）。
          // 读者要求至少减半，取 20vh（同窗口 161px ≈ 表头 + 约 4 行），超出部分仍由 body 内部滚动。
          '.dsh-fe-bar { max-height:20vh; animation:dsh-fe-bar-in .18s ease-out; }',
          ...
          // 浮层姿态不用 width 表达宽度，而是左右各内缩同一个 side-clearance：这样
          // left/right 决定宽度、max-width 仍封顶、两个 auto 外边距负责居中，三种约束
          // 不会互相打架（写成 left:0;right:0 + width 会让 right 被丢弃而左对齐）。
          '.dsh-fe-bar-overlay { position:absolute; left:var(--dsh-composer-side-clearance, 16px); right:var(--dsh-composer-side-clearance, 16px); width:auto; margin:0 auto; z-index:8; }',
          ...
          // —— 审核行的显示方式：文件名在前（主文字色），原始地址退成其后的小字次要色 ——
          // 只拆同一个路径字符串；完整路径仍留在行上的 title 里，地址一个字符都没丢。
          // 作用域限定在 .dsh-fe-path-split（普通文件行），目录删除批次的两行不受影响。
          '.dsh-fe-path-split { display:flex; align-items:baseline; gap:6px; min-width:0; }',
          '.dsh-fe-path-split > .dsh-fe-file-name { flex:0 1 auto; min-width:0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; color:var(--dsw-alias-label-primary); }',
          '.dsh-fe-path-split > .dsh-fe-file-dir { flex:1 1 0; min-width:0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; color:var(--dsw-alias-label-tertiary); font-size:10px; }',
          // —— 窄屏内容适配（2026-09-16）——
          // 审核栏宽度跟随官方输入框后可以被拖到很窄。一行里的固定项（文件图标、状态标签、
          // +/− 统计、两个操作钮）与 7 个 8px 间距合计约 220px，再窄就没有余量同时读地址和
          // 文件名了：先丢掉原始地址（完整路径仍在行的 title 里，一个字符都没少），文件名
          // 继续按上面的 flex 规则用省略号收尾。容器查询读的是审核栏自身的内容盒宽度。
          '@container (max-width:480px) { .dsh-fe-path-split > .dsh-fe-file-dir { display:none; } }',
          // —— 对齐官方右侧栏标签条（2026-09-16）——
          // 官方的 chip 是 28px 胶囊：radius 12、padding 0 10px、13px 字号、无边框；选中用
          // --dsw-alias-markdown-tag 填充 + 主文字色，hover 用 --dsw-alias-interactive-bg-hover，
          // chip 之间是 10px 分隔槽里的 0.5px 竖线（选中胶囊两侧的竖线隐去），右侧控件是 28px 圆钮。
          // 插件自己的文件标签条照这套几何与用色改写，使它和上方官方标签条读作同一种行。
          '.dsh-fe-filetabs { gap:4px; padding:10px 6px 0 10px; border-bottom:none; }',
          '.dsh-fe-filetabs-scroll { gap:10px; }',
          '.dsh-fe-filetab { position:relative; height:28px; padding:0 10px; border:none; border-radius:12px; font-size:var(--dsh-content-font-size-secondary, 13px); line-height:1; transition:background .12s ease, color .12s ease; }',
          '.dsh-fe-filetab::before { content:""; position:absolute; left:-5.25px; top:50%; width:0.5px; height:14px; background:var(--dsw-alias-border-l4); transform:translateY(-50%); }',
          '.dsh-fe-filetab:first-child::before, .dsh-fe-filetab-on::before, .dsh-fe-filetab-on + .dsh-fe-filetab::before { background:transparent; }',
          '.dsh-fe-filetab:hover { background:var(--dsw-alias-interactive-bg-hover); color:var(--dsw-alias-label-primary); }',
          '.dsh-fe-filetab-on, .dsh-fe-filetab-on:hover { background:var(--dsw-alias-markdown-tag); color:var(--dsw-alias-label-primary); }',
          '.dsh-fe-filetab .dsh-fe-tab-ic { color:inherit; }',
          '.dsh-fe-tab-x { width:20px; height:20px; padding:0; border:none; border-radius:20px; background:transparent; color:var(--dsw-alias-label-tertiary); opacity:0; pointer-events:none; transition:opacity .12s ease, background .12s ease, color .12s ease; }',
          '.dsh-fe-filetab:hover .dsh-fe-tab-x, .dsh-fe-filetab:focus-within .dsh-fe-tab-x, .dsh-fe-filetab-on .dsh-fe-tab-x { opacity:1; pointer-events:auto; }',
          '.dsh-fe-tab-x:hover { background:var(--dsw-alias-interactive-bg-hover); color:var(--dsw-alias-label-primary); }',
          '.dsh-fe-tab-controls { height:28px; gap:8px; margin-left:4px; }',
          '.dsh-fe-tab-more { width:28px; height:28px; border-radius:28px; }',
          // 官方面板是一整块连续的白底，标签条自己不画卡片描边：侧栏宿主里去掉浏览器的卡片边框与圆角，
          // 让标签行、工具行和内容共用面板的底色与左右内缩（10px / 6px，与官方标签条一致）。
          '.dsh-fe-sidebar-host .dsh-fe-viewer { border:none; border-radius:0; }',
          '.dsh-fe-sidebar-host .dsh-fe-filetabs { border-radius:0; box-shadow:inset 0 0.5px 0 var(--dsw-alias-border-l4), inset 0 -1px 0 var(--dsw-alias-border-l1); margin-top:12px; padding-top:12px; padding-bottom:11px; }',
          // 工具行自带 6px 上下内边距，分隔线两侧因此对称。不要再给工具行补上方外边距：
          // 历史上的 margin-top:5px 是按“工具行 5px 上内边距”写的，而实际是 6px，
          // 于是「修改」胶囊上方 13px、下方 8px（实测，2x 截图换算）。
          // 线色 = --dsw-alias-border-l1，在面板底上正是 #F6F6F6。
          // 文件浏览器里的审核按钮与审核列表同一套：圆形 + 官方交互底色，色调只落在墨色上。
        ...
        // 文件/文件夹图标现在由官方 ui-primitives 提供（见文件顶部 require），
        // 这里不再保留插件自绘的通用文件/文件夹字形。
        ...
        function TreeNode(props) {
          ...
          if (node.type === 'directory') {
            ...
            return React.createElement('div', null,
              React.createElement('div', { className: 'dsh-fe-row dsh-fe-dir' + (open ? ' dsh-fe-open' : ''), onClick: () => setOpen(!open) },
                ...
                React.createElement('span', { className: 'dsh-fe-ic' }, React.createElement(open ? IconFolderOpen16 : IconFolderClose16, { size: 15 })),
          ...
          return React.createElement('div', {
            ...
            React.createElement('span', { className: 'dsh-fe-ic' }, React.createElement(FileTypeIcon, { path: node.path, size: 15 })),
            React.createElement('span', { className: 'dsh-fe-name' }, node.name),
          )
        }
          ...
          return React.createElement('div', { className: 'dsh-fe-ws-item' },
            ...
            open ? React.createElement('div', null,
              ...
              React.createElement('div', {
                ...
                React.createElement('span', { className: 'dsh-fe-chev' }, IconChevron()),
                React.createElement('span', { className: 'dsh-fe-ic' }, React.createElement(secFiles ? IconFolderOpen16 : IconFolderClose16, { size: 15 })),
        ...
        function WorkspaceSidebar(props) {
          ...
          if (!wide) {
            return React.createElement('div', { className: 'dsh-fe-rail' },
              ...
              }, React.createElement(IconFolderClose16, { size: 15 }))),
            )
          }
        ...
        // ---------- modified files bar ----------
        // 审核行只改显示方式：文件名在前并给主文字色，原始地址退成其后的小字次要色。
        // 地址本身一个字符都没删（行上的 title 仍是完整路径），拆的只是同一个字符串。
        const splitReviewPath = (path) => {
          const full = String(path || '')
          const name = referenceFileName(full)
          const cut = full.length - name.length
          return { name, dir: cut > 0 ? full.slice(0, cut).replace(/[\\/]+$/, '') : '' }
        }
        function FileRow(props) {
          ...
          return React.createElement('div', { className: 'dsh-fe-file-row' },
            React.createElement('span', { className: 'dsh-fe-ic' }, React.createElement(FileTypeIcon, { path: item.path, size: 15 })),
            React.createElement('span', { className: 'dsh-fe-path dsh-fe-path-split', title: item.path, onClick: () => store.openFile(reviewId, props.sid, item.path, { firstChange: true }) },
              React.createElement('span', { className: 'dsh-fe-file-name' }, label.name),
              label.dir ? React.createElement('span', { className: 'dsh-fe-file-dir' }, label.dir) : null),
        ...
        function DeletionBatchRow(props) {
          ...
          return React.createElement('div', { className: 'dsh-fe-delete-batch' },
            React.createElement('div', { className: 'dsh-fe-file-row' },
              React.createElement('span', { className: 'dsh-fe-ic' }, React.createElement(IconFolderClose16, { size: 15 })),
            ...
            open ? React.createElement('div', { className: 'dsh-fe-delete-batch-children' },
              item.children.map((child) => React.createElement('div', { key: child.id || child.path, className: 'dsh-fe-delete-batch-child' },
                React.createElement('span', { className: 'dsh-fe-ic' }, React.createElement(FileTypeIcon, { path: child.path, size: 15 })),
...
//# sourceMappingURL=markdown-it.umd.min.js.map
        ...
        // ==== markdown source lines (begin) ====
        // 渲染视图的行级引用：给渲染出的 DOM 标注每段文字来自 .md 的哪一行，好把选中文本映射成
        // `path:起始行-结束行`。markdown-it 已经给出所需的全部位置：块级 token 带 `map`（0 基），
        // 多行段落的 inline 子 token 又被 `softbreak` 按源码行切开，所以「每个 text 片段属于哪一行」
        // 可以精确推出，不需要字符级 source map。
        // `text` 片段包一层 `<span data-line>`：选中时从 anchor/focus 节点就近取行号。
        const markdownSourceLines = (md) => {
          let line = 0
          const renderer = md.renderer
          const rules = renderer.rules
          const escapeHtml = md.utils.escapeHtml
          const defaultRenderToken = renderer.renderToken.bind(renderer)
          const defaultCodeInline = rules.code_inline
          const defaultFence = rules.fence
          // 块级起始标签带上本块首行，并把行计数器定位到它。
          ...
          // 每个 text 片段只属于一行：行内的换行由 softbreak 切开。
          rules.text = (tokens, idx) => '<span data-line="' + line + '">' + escapeHtml(tokens[idx].content) + '</span>'
          // 软/硬换行结束它所在的那一行。
          rules.softbreak = () => { line += 1; return '\n' }
          rules.hardbreak = () => { line += 1; return '<br>\n' }
          // 行内代码标注它自己所在的行：markdown-it 在解析时就把代码跨度里的换行折成了空格
          // （`content` 里已经没有换行），所以它只贡献一个行号；跨行的代码跨度会让随后的文本
          // 落在它的起始行上——漂移被限制在本块内，下一个块级标签会重新定位计数器。
          rules.code_inline = (tokens, idx, options, env, self) =>
            defaultCodeInline(tokens, idx, options, env, self).replace('<code', '<code data-line="' + line + '"')
          // 围栏代码块：正文从围栏的下一行开始；高亮输出本来就按行拼接，逐行标注即可。
          rules.fence = (tokens, idx, options, env, self) => {
            const token = tokens[idx]
            const start = (token.map ? token.map[0] : 0) + 2
            line = start
            const out = defaultFence(tokens, idx, options, env, self)
            const parts = /^(<pre[\s\S]*?<code[^>]*>)([\s\S]*?)(<\/code><\/pre>\s*)$/.exec(out)
            if (!parts) return out
            // 高亮输出以换行结尾，包裹后必须原样保留，否则 <pre> 的最后一行会少一个换行。
            const body = parts[2]
            const trailing = body.endsWith('\n') ? '\n' : ''
            const rows = body.replace(/\n$/, '').split('\n').map((text, i) => '<span data-line="' + (start + i) + '">' + text + '</span>')
            return parts[1] + rows.join('\n') + trailing + parts[3]
          }
        }
        // ==== markdown source lines (end) ====
        ...
        function WholeDocumentEditor(props) {
          ...
          // v1.14: 编辑视图里的目标行。审核列表与引用给的定位落到这里：选中整段目标行并滚到
          // 视野里；目标只消费一次，之后的刷新与输入不会再把视线拉走。不抢焦点——点击手势
          // 自己决定焦点去了哪里。
          const editorTargetEpoch = props.lineTarget ? props.lineTarget.epoch : 0
          React.useEffect(() => {
            if (!editorTargetEpoch || !viewRef.current || !props.lineTarget) return
            const target = store.claimFileTarget(props.lineTarget.path, props.lineTarget.firstChangeLine)
            if (!target) return
            const view = viewRef.current
            const doc = view.state.doc
            const from = doc.line(Math.min(Math.max(1, target.line), doc.lines))
            const to = doc.line(Math.min(Math.max(from.number, target.endLine), doc.lines))
            view.dispatch({ selection: { anchor: from.from, head: to.to }, scrollIntoView: true })
          }, [editorTargetEpoch, props.path])
        ...
        function DiffPane(props) {
          ...
          // 渲染视图的选区端点 → 源码行号：文本节点就近向上找渲染器标注的 `[data-line]`；
          // 端点落在元素上时（三击、拖到块尾）取该元素内部首/末一个标注，避免退化成整块的起始行。
          const insideMarkdown = node => (node?.nodeType === 1 ? node : node?.parentElement)?.closest?.('.dsh-fe-md')
          const markdownLineAt = (node, edge) => {
            const el = node && node.nodeType === 1 ? node : node && node.parentElement
            if (!el) return undefined
            const own = el.closest ? el.closest('[data-line]') : null
            if (node.nodeType === 1) {
              const inside = el.querySelectorAll ? el.querySelectorAll('[data-line]') : []
              if (inside.length) return Number((edge === 'end' ? inside[inside.length - 1] : inside[0]).dataset.line)
            }
            return own ? Number(own.dataset.line) : undefined
          }
          /** 渲染视图选区的行区间，取不到标注时返回 null。 */
          const markdownLineRange = (sel) => {
            const from = markdownLineAt(sel.anchorNode, 'start'), to = markdownLineAt(sel.focusNode, 'end')
            if (!from || !to) return null
            return { start: Math.min(from, to), end: Math.max(from, to) }
          }
          React.useEffect(() => {
            const markOf = node => (node?.nodeType === 1 ? node : node?.parentElement)?.closest?.('.dsh-fe-line')
            const onSelection = () => {
              const sel = window.getSelection?.()
              ...
              // 只有「有选区但不是 diff 行」时才不动：代码文件（CodeMirror）的选区由编辑器自己的
          ...
          // 只对带行号的视图生效：diff 视图与代码浏览靠 `.dsh-fe-line[data-n]`，
          // markdown 渲染视图靠渲染器标注的 `[data-line]`。
          const referenceDocumentLines = (startLine, endLine) => {
            const inserted = insertFileReference(fileReferenceInsert(path + ':' + startLine + '-' + endLine, 'file', referenceFileName(path)))
            if (!inserted) console.info('[dsh-file-edit] 行级引用未插入:', path, 'L' + startLine + '-L' + endLine)
            return inserted
          }
          React.useEffect(() => {
            ...
            const onKey = (ev) => {
              ...
              // 渲染视图：行号来自渲染器标注，引用走带行号的 codec（模型能看到这些行）。
              if (insideMarkdown(sel.anchorNode) && insideMarkdown(sel.focusNode)) {
                const rows = markdownLineRange(sel)
                if (!rows) return
                ev.preventDefault()
                ev.stopPropagation()
                referenceDocumentLines(rows.start, rows.end)
                return
              }
              ...
              ev.preventDefault()
              ev.stopPropagation()
              // 官方引用：与 @ 菜单同源，行区间只落在 chip 的显示名上。
              const referenced = insertOfficialReference({
                path: path,
                kind: 'file',
                label: referenceFileName(path) + ':' + (start === end ? String(start) : start + '-' + end),
              })
              if (!referenced) console.info('[dsh-file-edit] 选中行引用未插入:', path, 'L' + start + '-L' + end)
            }
          ...
          // 把一个行/块节点滚进视野：规则与 hunk 跳转一致——内层滚动容器不用偏移，
          // 外层聊天列要减去吸顶的标签栏与工具栏高度。
          ...
          // v1.14: 带目标行打开文件（引用或审核列表给的定位）。行节点两种写法都认：diff
          // 行与只读行是 data-n，Markdown 源码视图是 data-line。目标由 store 交付一次，
          // 所以之后的轮询重渲染、编辑、手动滚动都不会把滚动位置抢回去。
          const lineNodeFor = (line) => diffRef.node
            ? diffRef.node.querySelector('[data-n="' + line + '"], [data-line="' + line + '"]')
            : null
          const pendingTarget = store.fileTargets.get(path) || null
          const firstChangeLine = diff && Array.isArray(diff.hunks) && diff.hunks.length > 0
            ? diff.hunks[0].newStart + 1
            : (diff && Array.isArray(diff.current) && diff.current.length > 0 ? 1 : 0)
          const editorLineTarget = pendingTarget && documentMode === 'edit'
            ? { path: path, epoch: store.targetEpoch, firstChangeLine: firstChangeLine }
            : null
          React.useLayoutEffect(() => {
            if (!pendingTarget || documentMode === 'edit') return
            const wanted = pendingTarget.line > 0 ? pendingTarget.line : (pendingTarget.firstChange ? firstChangeLine : 0)
            if (!(wanted > 0) || !lineNodeFor(wanted)) return
            const target = store.claimFileTarget(path, firstChangeLine)
            if (!target) return
            const nodes = []
            for (let n = target.line; n <= target.endLine; n++) {
              const node = lineNodeFor(n)
              if (node) nodes.push(node)
            }
            if (nodes.length === 0) return
            scrollNodeIntoView(nodes[0])
            for (const node of nodes) node.classList.add('dsh-fe-target')
            setTimeout(() => { for (const node of nodes) node.classList.remove('dsh-fe-target') }, 1200)
          }, [path, diff, documentMode, store.targetEpoch])
          if (!diff) {
          ...
          // 官方引用：文件级 mention（`@path`），行区间只落在 chip 的显示名上，
          // 因为官方 file-reference 只解析路径（host 侧 sessionFileReferences 没有行区间）。
          const referenceDocumentSelection = (startLine, endLine) => {
            const name = referenceFileName(path)
          ...
          const selectionBubbleElement = () => {
            ...
            return React.createElement('button', {
              ...
              onClick: () => {
                if (bubble.lineRef) referenceDocumentLines(bubble.start, bubble.end)
                else referenceDocumentSelection(bubble.start, bubble.end)
            ...
            return React.createElement('div', { className: 'dsh-fe-pane' },
              ...
              React.createElement('div', { className: 'dsh-fe-document-wrap' },
                React.createElement(WholeDocumentEditor, {
                  ...
                  onSave: saveDocument,
                  // 只读浏览 / 编辑视图用编辑器自己报的行号走带行号的 codec：
                  // 官方 mention 只带路径，模型拿不到「第几行到第几行」。
        ...
        function FileView(props) {
          ...
          useStore()
          React.useEffect(() => { if (sid) store.setSessionId(sid) }, [sid])
          // 宿主声明浏览器此刻是否在屏幕上（侧栏标签可能被收成导轨或被切走）。
          // 会话视图宿主不声明，按“挂载即在屏幕上”处理，行为与原来一致。
          React.useEffect(() => {
            // 浮层姿态只属于"文件视图就是当前会话视图"这一种情形。侧栏宿主不是会话视图：
            // 打开右侧栏若把它置真，审核栏会变成绝对定位浮层并盖住上方的规划面板。
            if (props.hostConversationView === false) {
              store.setFileViewActive(false)
              return undefined
            }
            store.setFileViewActive(props.hostVisible === undefined ? true : !!props.hostVisible)
            return () => { store.setFileViewActive(false) }
          }, [props.hostVisible, props.hostConversationView])
          ...
          React.useEffect(() => {
            // 侧栏宿主下方没有对话的修改条，那个 dock 预留会变成纯粹的空白，按 0 处理。
            const clear = () => (props.hostDockless === true ? 0 : (store.dockH || 0))
            const apply = () => { if (viewerRef.node) viewerRef.node.style.setProperty('--dsh-fe-dock-h', clear() + 'px') }
            apply()
          ...
          return React.createElement('div', { className: 'dsh-fe-viewer', ref: (node) => { viewerRef.node = node } },
            React.createElement('div', { className: 'dsh-fe-filetabs', ref: (node) => { tabsRef.node = node } },
              React.createElement('div', { className: 'dsh-fe-filetabs-scroll', ref: tabScrollRef }, tabs.map((t, i) => React.createElement('span', {
                ...
                React.createElement('span', { className: 'dsh-fe-tab-ic' }, React.createElement(FileTypeIcon, { path: t, size: 15 })),
        ...
        function SidebarFileView(props) {
          ...
          useStore()
          const sid = props && props.sessionId !== undefined && props.sessionId !== null ? String(props.sessionId) : ''
          // store 是逐会话切换的：标签体刚挂载、setSessionId 还没落地时先不判断是否为空。
          const showingThisSession = store.sessionId === null || String(store.sessionId) === sid
          const docCount = store.tabs ? store.tabs.length : 0
          const visible = tabInfo ? !!tabInfo.tab.visible : true
          React.useEffect(() => {
            // 没有可浏览的文档就不留一个空白标签：入口只从文件点击产生。
            if (!tabInfo || docCount > 0 || !showingThisSession) return
            try { tabInfo.tab.actions.close() } catch (e) {}
          }, [tabInfo, docCount, showingThisSession])
          return React.createElement('div', {
            className: 'dsh-fe-sidebar-host',
            style: { height: '100%', minHeight: 0, display: 'flex', flexDirection: 'column' },
          }, React.createElement(FileView, {
            sessionId: props.sessionId,
            // 不传 hostVisible：侧栏宿主不驱动"文件视图是当前会话视图"这个标志。
            hostConversationView: false,
            hostDockless: true,
          }))
        }
        // 宿主注册：官方右侧栏可用时就只挂侧栏标签（会话里不再有常驻的「文件」标签）；
        // 那套服务始终不出现的旧产品，才回到会话视图宿主。
        let disposeConversationHost = null
        ...
        ctx.inject(['sidebarRightTabs'], (sidebarCtx) => {
          if (disposeConversationHost) { disposeConversationHost(); disposeConversationHost = null }
          ctx.effect(() => sidebarCtx.sidebarRightTabs.register({
