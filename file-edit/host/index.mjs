...
export default {
  ...
  apply(ctx, config = {}) {
    ...
    // `str_replace_editor` is opt-in in the kernel (a preset has to enable it),
    // but its mutating commands change files exactly like `write`/`edit`, so
    // they are mapped the same way. `view` and incomplete arguments are not
    // mutations and stay out of review. The three commands mirror the official
    ...
    // crashed tool) must not pin its pre-image forever; the oldest entry is
    // dropped once this many calls are in flight.
    const MAX_PENDING_STR_REPLACE = 64
    ...
    function resolvedShellAccess(exec) {
      ...
      // [权限门禁已禁用] 不再按工具种类或后台运行否决 Shell：本插件只做记录。
      if (!SHELL_TOOLS.has(name)) return { allowed: true, audit: 'none' }
      const session = exec?.agent?.session
      if (!session) return { allowed: true, audit: 'none' }
      try {
        ...
        if (!policy || typeof policy.mode !== 'string') return { allowed: true, audit: 'none' }
        if (policy.mode === 'read-only') return { allowed: true, audit: 'none', policy }
        if (!['workspace-write', 'danger-full-access'].includes(policy.mode)) return { allowed: true, audit: 'none', policy }
        ...
        // [权限门禁已禁用] 原本这里：没有“命令修改前事务”能力就拒绝全部 Shell。
        // 现在不否决，继续解析 auditRoot 并返回 audit:'transaction'，由
        // tools/execute 路由到 captureForegroundShell（执行前快照 + 执行后监听）。
        ...
        if (!isAbsoluteDiskPath(rooted)) return { allowed: true, audit: 'none', policy }
        let auditRoot
        try {
          ...
          return { allowed: true, audit: 'none', policy }
        }
        return { allowed: true, audit: 'transaction', policy, auditRoot }
      } catch (error) {
        return { allowed: true, audit: 'none' }
      }
    }
    function assertAgentFileMutationAllowed(exec, diskPath) {
      // [权限门禁已禁用] 删除/移动不再由本插件做策略判断：沙箱与审批归内核，
      // 本插件只负责记录改动。保留函数签名，调用点无需改动。
      return
    }
    ...
    // Pre-images of `str_replace_editor` calls, keyed by tool call id and
    // consumed by that call's result.
    const pendingStrReplace = new Map()
    ...
    function saveState(st, strict = false) {
      try {
        ...
        writeFileSync(temporary, JSON.stringify({ version: 7, root: st.root, baseReady: false, files, auditCoverage: st.auditCoverage ?? null, auditGaps: st.auditGaps ?? null, lastReject: st.lastReject ?? null }))
    ...
    function loadState(sid) {
      try {
        ...
        const auditCoverage = data.auditCoverage?.kind === 'partial'
          ? { kind: 'partial' } : null
        const auditGaps = sanitizeAuditGaps(data.auditGaps)
        return { version: persistedVersion, root: data.root ?? null, baseReady: data.baseReady === true, files, lastReject, auditCoverage, auditGaps }
      } catch (e) {
        return null
      }
    }
    ...
    function newState(sid) {
      ...
      return {
        ...
        auditGaps: restored?.auditGaps ?? null,
    ...
    // ---------- 审核归属：会话 → 工作区根 ----------
    // 工作区可以嵌套（例如 ~/Downloads 里还挂着另一个工作区）。被动观察到的改动（shell 命令
    // 前后的整根比对、工作区遍历）只有落在**本会话**的工作区根里才算本会话的：落在别的会话根
    // 里的改动由那个会话自己的工具归因认领，否则会在它那边并入 baseline。显式工具写入
    // （write/edit/file_*）不走这条规则——那是调用方明确做的事，按 exec.agent.session.id 归因。
    const SESSION_ROOT_TTL = 5000
    let sessionRootIndex = { at: 0, roots: [], owners: new Map() }
    let sessionRootRefresh = null
    function normalizeOwnerRoot(value) {
      const resolved = normalizedDiskPath(value)
      try { return realpathSync(resolved) } catch (e) { return resolved }
    }
    // 索引不可用（list() 失败）时退回状态目录：每个会话文件的 root 字段就在最前面。
    function sessionRootsFromStateDir() {
      const pairs = []
      try {
        for (const name of readdirSync(STATE_DIR)) {
          if (!/^session-.*\.json$/.test(name)) continue
          const head = readFileSync(join(STATE_DIR, name), 'utf8').slice(0, 400)
          const match = /"root"\s*:\s*"((?:[^"\\]|\\.)*)"/.exec(head)
          if (!match) continue
          try { pairs.push([name.replace(/^session-/, '').replace(/\.json$/, ''), JSON.parse('"' + match[1] + '"')]) } catch (e) {}
        }
      } catch (e) {}
      return pairs
    }
    async function refreshSessionRootIndex() {
      const now = Date.now()
      if (now - sessionRootIndex.at < SESSION_ROOT_TTL && sessionRootIndex.roots.length > 0) return sessionRootIndex
      if (sessionRootRefresh) return sessionRootRefresh
      sessionRootRefresh = (async () => {
        let pairs = []
        try {
          const value = await sessions.list({}, new AbortController().signal)
          for (const item of (value && value.items) || []) {
            if (!item || !item.id || typeof item.cwd !== 'string' || !item.cwd) continue
            pairs.push([String(item.id), item.cwd])
          }
        } catch (e) { pairs = [] }
        if (pairs.length === 0) pairs = sessionRootsFromStateDir()
        const owners = new Map()
        for (const [rawSid, rawRoot] of pairs) {
          const owner = reviewOwnerSessionId(rawSid)
          if (!owner) continue
          const root = normalizeOwnerRoot(rawRoot)
          if (!root) continue
          if (!owners.has(root)) owners.set(root, new Set())
          owners.get(root).add(owner)
        }
        sessionRootIndex = { at: Date.now(), roots: [...owners.keys()].sort((a, b) => b.length - a.length), owners }
        return sessionRootIndex
      })().finally(() => { sessionRootRefresh = null })
      return sessionRootRefresh
    }
    /** @returns 该磁盘路径所属的工作区根拥有者集合；null = 无主（工作区之外）；undefined = 索引未知。 */
    function ownersOfDiskPath(absPath) {
      const index = sessionRootIndex
      if (index.roots.length === 0) return undefined
      const disk = normalizeOwnerRoot(absPath)
      for (const root of index.roots) {
        if (disk === root || disk.startsWith(root.endsWith('/') ? root : root + '/')) return index.owners.get(root)
      }
      return null
    }
    /** 被动观察到的改动是否属于别的会话；索引未知时返回 false（保持原有行为）。 */
    function isForeignSessionPath(sid, absPath) {
      const owners = ownersOfDiskPath(absPath)
      if (owners === undefined || owners === null) return false
      return !owners.has(String(sid))
    }
    /** 命令自身是否提到过这条相对路径（提到就说明是本命令的产物，即使它在别的工作区里）。 */
    function commandCoversRelPath(command, rel) {
      ...
      if (head && head.length > 2 && text.includes(head)) return true
      return segments.length > 1 && text.includes(segments[0] + '/')
    }
    /** 清掉旧版本按"命令窗口"误捕进来的跨工作区条目，返回清掉的条数。 */
    function pruneForeignShellEntries(st, sid) {
      let pruned = 0
      for (const [rel, entry] of [...st.files]) {
        if (reviewTargetIsExternal(rel)) continue
        const base = entry && entry.base
        // 只处理 shell 被动捕获的那类（base.version 前缀 shell:）；显式工具写入
        // （event: 前缀）即使落在别的工作区，也是本会话明确做的事，保留。
        if (!base || typeof base.version !== 'string' || !base.version.startsWith('shell:')) continue
        if (!st.root || !isForeignSessionPath(sid, reviewDiskPath(st, rel))) continue
        st.files.delete(rel)
        pruned++
      }
      if (pruned > 0) {
        console.info('[dsh-file-edit] 已移除本会话中 ' + pruned + ' 条属于其它会话工作区的审核条目')
        st.needsCompact = true
      }
      return pruned
    }
    ...
    async function hydrateReviewLedger(sid) {
      ...
      const task = (async () => {
        ...
        st.error = null
        // 归属索引就绪后先清理跨工作区的旧条目，再水合剩余的待审核目标。
        await refreshSessionRootIndex()
        if (pruneForeignShellEntries(st, sid) > 0) saveState(st)
    ...
    function sanitizeAuditGaps(raw) {
      if (!raw || typeof raw !== 'object') return null
      const backgroundShell = Number.isFinite(raw.backgroundShell) && raw.backgroundShell > 0
        ? Math.floor(raw.backgroundShell)
        : 0
      const untrackedShell = Array.isArray(raw.untrackedShell)
        ? raw.untrackedShell.filter((name) => typeof name === 'string' && name !== '')
        : []
      if (backgroundShell === 0 && untrackedShell.length === 0) return null
      return { backgroundShell, untrackedShell }
    }

    // Shell entry points whose writes never enter the ledger: a background
    // command outlives the observation window, and the untracked compatibility
    // shells are not instrumented at all. The call still runs; the gap is
    // recorded so the review UI can state that its list is not complete
    // evidence. Persisted with the session so a restart keeps the disclosure.
    function noteAuditGap(sid, kind, tool) {
      if (!sid) return
      const st = initializeEventState(sid)
      const gaps = sanitizeAuditGaps(st.auditGaps) ?? { backgroundShell: 0, untrackedShell: [] }
      if (kind === 'background-shell') {
        gaps.backgroundShell += 1
      } else if (typeof tool === 'string' && tool !== '' && !gaps.untrackedShell.includes(tool)) {
        gaps.untrackedShell.push(tool)
      }
      st.auditGaps = gaps
      saveState(st)
    ...
    function noteAuditGapForTool(exec, kind) {
      const name = exec && exec.name ? String(exec.name) : ''
      noteAuditGap(reviewOwnerSessionId(exec?.agent?.session?.id), kind, name)
    }

    // A gap that cannot be written must not fail the shell call it describes.
    function noteAuditGapQuietly(exec, kind) {
      try {
        noteAuditGapForTool(exec, kind)
      } catch (e) {
        console.error('[dsh-file-edit] failed to record the audit gap:', e && e.message ? e.message : e)
      }
    }
    ...
    // Only the mutating commands of `str_replace_editor` are mapped; `view`
    // reads and an incomplete argument list are not mutations.
    function strReplaceMutation(exec) {
      const args = exec && exec.arguments
      if (!args || typeof args !== 'object' || Array.isArray(args)) return null
      if (!STR_REPLACE_MUTATIONS.has(args.command)) return null
      const path = typeof args.path === 'string' && args.path.trim() !== '' ? args.path : null
      return path ? { path, command: args.command } : null
    }

    // `str_replace_editor` answers with a message rather than with the
    // before/after text that `write`/`edit` return, so its baseline is read
    // here, before the call runs. A denied, failed, or never-answered call
    // leaves its entry unused.
    async function beginStrReplaceEdit(exec) {
      const mutation = strReplaceMutation(exec)
      if (!mutation) return
      ...
      if (!sid) return
      const st = initializeEventState(sid)
      const rel = reviewKeyFromRaw(st, mutation.path)
      if (!rel) return
      const before = mutation.command === 'create' ? null : await loadFileEntry(st, rel)
      if (pendingStrReplace.size >= MAX_PENDING_STR_REPLACE) {
        const oldest = pendingStrReplace.keys().next()
        if (!oldest.done) pendingStrReplace.delete(oldest.value)
      }
      pendingStrReplace.set(callId, { sid, rel, command: mutation.command, before })
    }

    async function finishStrReplaceEdit(exec, result) {
      const callId = exec.callId === undefined || exec.callId === null ? '' : String(exec.callId)
      if (!callId) return false
      const pending = pendingStrReplace.get(callId)
      if (!pending) return false
      pendingStrReplace.delete(callId)
      if (result && result.isError === true) return false
      const st = initializeEventState(pending.sid)
      const stamp = (st.mutationStamp || 0) + 1
      st.mutationStamp = stamp
      // `create` starts from ABSENT, and so does any command whose pre-image
      // read found no file. The other commands compare against the pre-image;
      // an unreadable one keeps an explicitly unrestorable baseline instead of
      // claiming the file was new.
      const before = pending.command === 'create' || (pending.before && pending.before.present === false)
        ? absentEntry()
        : (pending.before ? pending.before : unknownWriteBefore(reviewDisplayPath(st, pending.rel), stamp))
      const after = await loadFileEntry(st, pending.rel)
      if (!after.present) return false
      stageEntries(st, pending.rel, before, after)
      st.dirty = false
      saveState(st)
      scheduleNotify(pending.sid, 80)
      return true
    }
    ...
    function snapshotShellWorkspacePaths(st, sid) {
      const files = new Map()
      let saturated = false
      const walk = (diskDir, relDir, depth) => {
        ...
        for (const entry of entries) {
          ...
          if (sid && isForeignSessionPath(sid, disk)) continue
          let info
    ...
    async function captureForegroundShell(exec, next) {
      ...
      const st = initializeEventState(sid)
      // [权限门禁已禁用] 监听式抓取本质上是“部分覆盖”：监听器可能丢事件，
      // 工作区之外的改动也不在范围内。如实告知界面，但不再因此否决命令。
      if (st.auditCoverage === null || st.auditCoverage === undefined) {
      ...
      // 归属索引先就绪：下面按"路径属于哪个会话的工作区根"过滤观察到的改动。
      await refreshSessionRootIndex()
      const shellCommand = exec && exec.arguments && typeof exec.arguments.command === 'string' ? exec.arguments.command : ''
      let foreignSkipped = 0
      const changed = new Set()
      const before = new Map()
      const beforeWorkspace = snapshotShellWorkspacePaths(st, sid)
      ...
      try {
        const watcher = watchFs(st.root, { recursive: true }, (_event, filename) => {
          ...
          if (rel && !ignoredReviewPath(rel)) {
            if (isForeignSessionPath(sid, resolvePath(st.root, rel)) && !commandCoversRelPath(shellCommand, rel)) foreignSkipped++
        ...
        // [权限门禁已禁用] 原先“建立不了审核事务”会直接拒绝执行命令。
        // 现在降级为：本次改动不抓取，但绝不否决。
        for (const watcher of watchers) { try { watcher.close() } catch (e) {} }
        console.error('[dsh-file-edit] shell audit watcher unavailable (capture skipped):', error && error.message ? error.message : error)
        return next()
      }
      ...
      const afterWorkspace = snapshotShellWorkspacePaths(st, sid)
      ...
      for (const rel of changed) {
        // 掉进别的工作区的改动不是本会话的审核内容（命令显式提到该路径时除外）。
        if (isForeignSessionPath(sid, resolvePath(st.root, rel)) && !commandCoversRelPath(shellCommand, rel)) {
          foreignSkipped++
          continue
        }
        const old = st.files.get(rel)
      ...
      if (foreignSkipped > 0) {
        console.info('[dsh-file-edit] shell 观察到的 ' + foreignSkipped + ' 处改动属于其它会话的工作区，未计入本会话审核')
      }
      if (staged > 0) {
        st.dirty = false
        saveState(st)
      ...
      if (watcherError && !thrown) {
        // [权限门禁已禁用] 原先这里会抛错，导致“命令已成功但工具报失败”。
        // 记录器的健壮性问题不应该伪装成执行失败，改为告警。
        console.error('[dsh-file-edit] shell audit incomplete after command:', watcherError && watcherError.message ? watcherError.message : watcherError)
      }
    ...
    async function captureTransactionalShell(exec, next, resolvedAccess) {
      ...
      try {
        settled = await SHELL_SNAPSHOT_TRANSACTIONS.run({
          ...
          try { await settleTransactionalShellLedger(sid, captured, exec && exec.arguments && typeof exec.arguments.command === 'string' ? exec.arguments.command : '') } catch (error) {
    ...
    async function settleTransactionalShellLedger(sid, settled, commandText = '') {
      const st = initializeEventState(sid)
      await refreshSessionRootIndex()
      const transactionCommand = String(commandText || '')
      // 事务快照覆盖整个 audit root，同样要把别的工作区里的产物挡在外面。
      const foreign = (relativePath, rootIndex) => {
        const abs = resolvePath(settled.transaction.roots[rootIndex], relativePath)
        return isForeignSessionPath(sid, abs) && !commandCoversRelPath(transactionCommand, relativePath)
      }
      const manifest = SHELL_SNAPSHOT_TRANSACTIONS.manifestOf(settled.transaction)
      const deletionRoots = topLevelShellDeletions(settled.changes).filter((root) => !foreign(root.relativePath, root.rootIndex))
      ...
      for (const change of settled.changes) {
        ...
        if (foreign(change.relativePath, change.rootIndex)) continue
    ...
    async function scan(sid) {
      ...
      const task = (async () => {
        try {
          ...
          const walked = []
          await walkFiles(rootTarget, '', walked, 0, { n: 0 }, sid, res.root)
          const seen = new Set()
    ...
    async function walkFiles(dirTarget, rel, out, depth, count, sid, root) {
      if (depth > MAX_DEPTH || count.n >= MAX_ENTRIES) return
      // 别的工作区（可以嵌套在本根里）不进入本会话的清单：既不遍历，也不建立基线。
      if (sid && root && rel && isForeignSessionPath(sid, resolvePath(root, rel))) return
      let entries
      ...
      for (const e of entries) {
        ...
        if (e.type === 'file') {
          ...
          if (sid && root && isForeignSessionPath(sid, resolvePath(root, fileRel))) continue
          out.push({ rel: fileRel, version: version, size: size })
        }
      }
      for (const e of entries) {
        ...
        if (e.type === 'directory') {
          ...
          await walkFiles(e.target, rel ? rel + '/' + e.name : e.name, out, depth + 1, count, sid, root)
        }
      }
    }
    ...
    // [权限门禁已禁用] 原先这里注册 tools.guard(shellGateReason)，即使前面的
    // 策略瀑布全部放行，也会否决“不可追踪的 Shell 生命周期”。现在不再注册。
    ctx.effect(() => systemPrompt.section({
      name: 'tool:file-review-ledger',
      order: 103,
      text: 'File review is enabled. This plugin only RECORDS file changes for review and never blocks, denies, or gates tool execution: sandbox mode and approval policy are owned by the harness. Prefer write/edit for precise text changes, file_delete for recoverable deletion, and file_move for moving or renaming a file. Foreground bash/shell/pwsh calls in a writable workspace are captured by snapshotting the workspace before the call and watching it until the call settles. Changes made by background Shell, persistent terminals, or unmanaged compatibility entries may not be recorded, changes outside the snapshot directory may be unrecorded, and concurrent external changes cannot be reliably attributed. Do not treat an empty review list as evidence that no changes happened.',
    ...
    const api = {
      ...
      async getModified(args) {
        ...
        return { ok: true, root: st.root, files: modifiedFiles(st), auditCoverage: st.auditCoverage ?? null, auditGaps: st.auditGaps ?? null, treeStamp: st.treeStamp, undo: st.lastReject ? { opId: st.lastReject.opId, count: st.lastReject.files.length, ts: st.lastReject.ts } : null }
      },
    ...
    ctx.effect(() => () => {
      ...
      notifyTimers.clear()
      pendingStrReplace.clear()
    }, 'dsh-file-edit: wait cleanup')

    // ---------- change triggers ----------
    ctx.on('tools/pre-execute', async (exec, next) => {
      // [权限门禁已禁用] 原先这里会 `return { kind: 'deny', reason }`。
      // 现在本插件只观察、不否决任何工具调用。
      try {
        await beginStrReplaceEdit(exec)
      } catch (e) {
        // A baseline that cannot be read must not stop the tool call; the row
        // falls back to an unrestorable baseline when the result arrives.
        console.error('[dsh-file-edit] failed to read the str_replace_editor baseline:', e && e.message ? e.message : e)
      }
      return next()
    })
    ...
    ctx.on('tools/execute', async (exec, next) => {
      const name = exec && exec.name ? exec.name : ''
      // Two shell entry points never enter the ledger: a background command
      // outlives the observation window, and the compatibility shells are not
      // instrumented. The call still runs, but the gap is recorded so the
      // review UI can say its list is not complete evidence.
      if (UNTRACKED_SHELL_TOOLS.has(name)) {
        noteAuditGapQuietly(exec, 'untracked-shell')
        return next()
      }
      if (!SHELL_TOOLS.has(name)) return next()
      const shellArgs = exec && exec.arguments && typeof exec.arguments === 'object' ? exec.arguments : {}
      if (shellArgs.run_in_background === true) {
        noteAuditGapQuietly(exec, 'background-shell')
        return next()
      }
      const access = resolvedShellAccess(exec)
    ...
    ctx.on('tools/result', async (exec, result) => {
      ...
      try {
        ...
        await stageTextToolResult(exec, result)
      } catch (e) {
